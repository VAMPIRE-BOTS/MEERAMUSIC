...
import requests
import random
from AarumiMusic import app, userbot
...
@app.on_message(filters.command(["arumi","aru","aby", "abu", "ute"], prefixes=["A", "B", "a", "b", "c", "C"]) & admin_filter)
async def restriction_app(app :app, message):
    reply = message.reply_to_message
    chat_id = message.chat.id
    if len(message.text) < 2:
        return await message.reply(random.choice(Aarumi_text))
